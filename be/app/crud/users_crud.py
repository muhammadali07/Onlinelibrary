...
async def update_user_by_email(data,db: AsyncSession):
    async with db as session:
        try:
...
async def delete_user_by_email(data,db: AsyncSession):
    async with db as session:
        try:
            users = users_model.Users
            ...
            if result in (None, []):
                return ResponseOutCustom(message_id="02", status=f'Data user dengan {data} tidak ditemukan', list_data=[])
